...
import com.esri.android.geotrigger.MessageReceiver;
 ...
 */
public final class GeotriggerHelper extends MessageReceiver{
    private static final String TAG4 = "GeotriggerHelper";
    ...
    public static void startGeotriggerService(final Activity activity, int requestCode, String clientId,
        ...
        if (status != ConnectionResult.SUCCESS && !sDeclinedPlayServicesInstall) {
            ...
            if (GooglePlayServicesUtil.isUserRecoverableError(status)) {
                ...
                Log.d(TAG4, "Google Play Services not available, and cannot be installed on this device.");
                // Geotriggers can still work, using an older, less battery-efficient mode of operation.
            }

        } else {
            Log.d(TAG4, "Google Play Services is available (or user has declined to install it). " +
            ...
            if (checkForRequiredProviders(activity) == AvailableProviders.BOTH) {
                ...
                GeotriggerService.init(activity, clientId, tags, profile);
                //GeotriggerService.setOfflineProcessingEnabled(activity);
                //GeotriggerService.requestOnDemandTriggerDownload(activity);
                GeotriggerService.setTriggerDownloadInterval(activity, 300);
                GeotriggerService.setTriggerReportUploadInterval(activity, 300);

                /**
                  *if (GeotriggerService.isOfflineProcessingEnabled (activity)){
                    Toast.makeText(activity, "Offline processing enabled!",
                            Toast.LENGTH_LONG).show();
                  *}
                  *if(GeotriggerService.isPushNotificationHandlingEnabled (activity)){
                  *  Toast.makeText(activity, "Notification handling enabled!",
                            Toast.LENGTH_LONG).show();
                  * }
                  */
                String icon = "drawable/stamp";
                String sound = "raw/alert";
                GeotriggerService.setPushIcon(activity, icon);
                GeotriggerService.setPushSound(activity, sound);
            } else {
                Log.d(TAG4, "Delaying the start of Geotriggers, as we are awaiting the availability of " +
    ...
    private static AvailableProviders checkForRequiredProviders(final Context context) {
        ...
        if (!gotGps && !gotNetwork) {
            ...
            Log.d(TAG4, "Both GPS and Network providers are available.");
